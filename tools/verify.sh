...
for p in $pages; do
  ...
  code=$(curl -s -o /tmp/page.html -w '%{http_code}' "$url"); head=$(curl -s -o /dev/null -I -w '%{http_code}' "$url")
...
printf 'thanks noindex: %s\n' "$(curl -s "$BASE/thanks$EXT" | grep -c 'noindex')"
